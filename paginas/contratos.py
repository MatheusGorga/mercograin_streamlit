...
import pandas as pd
from funcoes_compartilhadas import conversa_banco, trata_tabelas, estilos, gerador_pdf # NOVO: import gerador_pdf
from datetime import datetime

# (O início do arquivo com as configurações e a função carregar_dados_apoio continua o mesmo)
...
def app():
    ...
    # (Toda a parte de mapas e formulário de criação continua a mesma)
    ...
    if not df_contratos.empty:
        ...
        for col, map_data in [("Comprador", dados_apoio["compradores"]), ("Vendedor", dados_apoio["vendedores"]), ("Produto", dados_apoio["produtos"]), ("Status", dados_apoio["status_contrato"])]:
            ...
            df_display[col] = df_display[f"ID_{col}"].map(map_series)
        ...
        df_editado = st.data_editor(df_display_final, column_config={"ID": None, "Selecionar": st.column_config.CheckboxColumn(required=True)}, disabled=df_display_final.columns.drop("Selecionar"), hide_index=True, use_container_width=True, key="grid_contratos")
        ids_selecionados = df_editado[df_editado["Selecionar"]]["ID"].tolist()

        if ids_selecionados:
            with st.container(border=True):
                st.write(f"**{len(ids_selecionados)} contrato(s) selecionado(s)**")
                # NOVO: Coluna para o botão de Gerar PDF
                ...
                if col_delete.button("🗑️ Deletar Contratos Selecionados", use_container_width=True):
                    ...
                    for id_contrato in ids_selecionados:
                        if conversa_banco.delete(TABELA_CONTRATOS, id_contrato): sucesso += 1
                ...
                if len(ids_selecionados) == 1:
                   ...
                   # NOVO: Lógica para preparar e gerar o PDF
                    try:
                        # Prepara o dicionário de dados para o template
                        dados_para_template = {
                        ...
                        # Constrói o caminho absoluto para a pasta de templates
                        PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
                        TEMPLATE_DOCX_PATH = os.path.join(PROJECT_ROOT, "templates", "Modelo de contrato Word - Copia.docx")
                        BACKGROUND_PNG_PATH = os.path.join(PROJECT_ROOT, "templates", "background.png")
                        
                        # Gera o PDF em memória usando os caminhos absolutos
                        pdf_bytes = gerador_pdf.criar_contrato_pdf(
                            template_path=TEMPLATE_DOCX_PATH,
                            background_image_path=BACKGROUND_PNG_PATH,
                    ...
                    with st.form("form_edita_contrato"):
                        ...
                        if st.form_submit_button("💾 Salvar Alterações"):
                            ...
                            if conversa_banco.update(TABELA_CONTRATOS, st.session_state['editando_contrato_id'], dados_atualizados):
                                ...
                                st.rerun()
                            else: st.error("❌ Falha ao atualizar o contrato.")
    else:
